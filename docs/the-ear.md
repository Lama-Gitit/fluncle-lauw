...
| `catalogue_rank_corpus`    | the corpus fingerprint the values above were computed against, `"<logic-version>:<findings>:<embedded>:<edges>:<enabled>:<disabled>"`. Staleness. |
...
Staleness has two halves. The **corpus half** is a fingerprint stored on every ranked row: `"<version>:<findings>:<embedded findings>:<track_artists edges>:<enabled labels>:<disabled labels>"`. The two finding counts move whenever the corpus side could change (log a finding, or embed one). The **three graph counts** move whenever AUTHORIZATION could change (RFC artist-primary-capture): the `track_artists` edge count grows as the graph backfill drains — so a row ranked before its edge existed re-derives its tier under the new gate — and the `enabled`/`disabled` label counts move on a ruling. A row whose stored fingerprint differs from the live one is stale and re-ranks on a later tick. A leading logic version (`v4`) forces one self-healing full re-rank when the sweep's algorithm itself changes.
...
`capture_priority` is the pre-audio answer — the cheap metadata signals that _can_ be read before a single byte is downloaded. It carries **two decisions, cleanly separated** (RFC artist-primary-capture, `docs/rfcs/artist-primary-capture-rfc.md`): **AUTHORIZATION** — may we spend a metered per-GB byte on this row at all? — and **PRIORITY** — among the rows we may buy, who first? Discovery is label-driven (the crawler seeds from `enabled` labels); **capture is artist-driven**. An artist who moved from a scene label to a major arrives by himself, and his other tunes deserve the spend even though the major is not a seed.
...
**Why authorization moved off the label.** `label` (a finding on the row's label) used to be an _authorizer_ — tier 2, into the budget. That is wrong: a finding is evidence about an **artist**, not a licence to buy a label's whole catalogue. The live counter-example is one Atlantic-UK finding that lifted **every** crawled Atlantic-UK track to tier 2. So `findingLabels` is now a priority **hint only** — an un-`enabled` label carrying a lone crossover finding no longer authorizes its label-mates.
...
**The veto is not decoration, and it was caught on real data.** Every one of the operator's 8 **disabled** labels — Anjunabeats, Armada, Axtone, Positiva … — _carries a finding_: each arrived on a single crossover remix. It is checked **first**, before authorization, so a qualified artist doing a remix on a ruled-out label still sinks: he _told_ us the label is not his lane. And overruling a duplicate verdict (`force_capture`, § Duplicates) is an operator authorization that lifts the artist gate too — but never the disabled-label veto.
...
So the sweep flags a duplicate, and it never gets bought. There are **several detectors**, because the identity signal changes the moment a row has a vector, and no single signal catches every copy:
...
4. **MatchKey-vs-findings — the twin the ISRC and the score both miss (the 2026-07-15 "Drifting Away" ruling).** Detectors 1–2 compare a catalogue row against the findings by ISRC and by near-1.0 score, and both slip through a YouTube rip of an already-logged track: it carries **no ISRC** and embeds to a merely-**0.94** twin, far below the 0.995 band. The crawled "Drifting Away — BOP, Unquote" ranked **0.94** against the finding **012.8.0A** ("BOP, Unquote — Drifting Away"), the exact same song, and sat at the top of the ear lens as a discovery. The ruling: a row whose folded title+artist `matchKey` (track-match.ts, the same matcher the Rekordbox sync uses) EQUALS a certified finding's is **that recording — a duplicate regardless of ISRC and regardless of embedding score**. It stamps the same `duplicate_of_track_id` + tier **−2** as the ISRC match, on **both** sides of the audio boundary: an uncaptured row is vetoed off the capture queue before a byte is bought (`preAudioPriority`, after the ISRC probe misses), and a scored row is marked **inside the scoring path** — keeping its vector and score (it still reads "already in the archive"), while its existing `duplicate_of_track_id is null` ear filter drops it from a ranked slot. The corpus is `readFindingMatchKeys`, the inverse of the wrong-audio `readFindingIdentities` map (matchKey → the **smallest** finding `track_id`, the same stable canonical precedent the catalogue-internal detector uses), read on **every** tick with candidates because a twin can hide at any score, not just the near-1.0 band. Because `matchKey` keeps a remix/VIP descriptor distinct, an original of a logged VIP — or a VIP of a logged original — is a **real discovery**, never merged. A finding match wins over a catalogue-internal one (the certified row is the canonical the board would rather name). The ranking-logic version is bumped to `v3` so every already-ranked row re-ranks once and the live 0.94 twin is re-marked without a manual invalidation.
...
**The description is personalized** and stays in the crew-facing register (VOICE.md): _"Dug for @<username> from the archive. Fresh ones every week. fluncle.com"_ — sentence case, no em dashes, ≤300 chars.
