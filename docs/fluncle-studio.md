...
## The one mixtape-scoped cue write-path

- **`set_mixtape_cues` — the all-or-nothing cue-sheet backfill** (`PUT /admin/mixtapes/{mixtapeId}/cues`, operator tier). Requires exactly one cue per member, start-at-0, strictly monotonic — the CLI full-backfill path (`setMixtapeCues`). It is the only write that re-times a minted mixtape's `start_ms` directly; everything else reaches `mixtape_tracks.start_ms` through the recording (`replace_recording_cues` → `promote`).

There used to be a second, interactive one: **`update_mixtape_cue`** (`PUT /admin/mixtapes/{mixtapeId}/cues/{ref}`), the single-cue upsert behind the mixtape-scoped rail. It is **retired** — see below.

## The mixtape-scoped rail — RETIRED

**There is no mixtape-scoped cue rail, and no single-cue op behind it.** `StudioCueRail` (`apps/web/src/components/admin/studio-cue-rail.tsx`) was the mixtape-scoped marking rail; the finding-linked `RecordingCueRail` superseded it as the Studio's cue editor, it sat with zero importers, and it was deleted. Its write op, **`update_mixtape_cue`**, never found another caller, so it was **retired on 2026-07-27 by operator ruling** — a live admin-API endpoint nobody calls is surface to defend for nothing. Removed end to end: the contract op, the handler + its router registration, the `setMixtapeCue` server function and its `mixtapes-cue.test.ts`, and the coverage/auth-tier entries. Git history keeps all of it; if the admin overhaul ever designs a mixtape-scoped rail, the op is rebuilt from there.

The one capability that died with it is **drop snapping** — a mark snapping to the nearest `StudioPeak` drop within a small window (`snapCueToPeak` / `CUE_SNAP_WINDOW_MS` in `apps/web/src/lib/studio-clip.ts`, still exercised by `studio-clip.test.ts` but by no UI). It was a mixtape-rail feature because it reads the mixtape's energy envelope; the recording rail carries no energy envelope, so the operator marks the exact playhead. If snapping is wanted back, it belongs on `RecordingCueRail` — the helpers are there and tested. They are **deliberately retained** through the op's retirement: they are pure, cheap, covered, and UI-agnostic, so they cost nothing to keep and would have to be rewritten from scratch otherwise.
