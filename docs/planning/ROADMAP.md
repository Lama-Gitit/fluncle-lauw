...
- **Then go-to-market:** promoting Fluncle in the DnB communities (Reddit and friends), reaching out to record labels (the groundwork is _Next → Label outreach_), and showing up where the scene is — the **September festival** is the first dated beat, with a batch of printed stickers ready to spread (the collateral has print lead time and does not wait on the overhaul — see _Next → Printed collateral_). The mobile app going live slots in wherever Apple's approval lands.
...
The Expo app ships feed, archive, finding pages, submissions, the Radio (background audio + lock-screen presence), Mixtapes, search, device-local saves, push toggles, and Apple Music links. 1.0 was submitted, **rejected under 5.2.3** (music rights), remediated (`97897ed7` — the feed video is a MUTED visual, the card's sound is the official ~30s preview, `hasAudio: false` pinned in the `CardMedia` type and asserted as the 5.2.3 invariant), resubmitted 2026-07-21 — then **rejected a second time 2026-07-28 under 5.2.1**: album covers in the STORE SCREENSHOTS (the binary itself passed). The operator removed the cover-led preview shot and replied the same day with a justification (music apps on the store show covers in list screenshots). **Next state change is Apple's.** If they hold the 5.2.1 line, the ready-made fallback is the own-IP screenshot rig in closed PR #976 (synthetic sleeve/avatar compositions, a guarded local-only seed, the capture runbook) — reopen, merge, re-shoot. Runbook: [docs/mobile-release.md](../mobile-release.md); posture: [docs/app-store-review.md](../app-store-review.md).
