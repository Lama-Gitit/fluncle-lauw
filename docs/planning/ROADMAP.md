...
- **The admin overhaul's waiting consumers.** Two deferrals already name the admin overhaul as what unblocks them: the per-row "generate" action on the observation dialogs (`apps/web/src/components/admin/observation-dialogs.tsx` — deferred to the overhaul, which decides where a per-row action belongs across every station at once, per [docs/admin-shell.md](../admin-shell.md) § placement contract), and the caller-less `update_mixtape_cue` op (_Next → Housekeeping follow-ups_).
...
Dependabot reports **1 critical, 6 high, 8 medium, 3 low** open against the repo (counted this date; the audit ledger's older row said 13 and is stale in both directions). Triage to zero-high, then decide the standing policy — what severity auto-merges, what waits for `minimumReleaseAge`, and who reads the queue. Ledger row: `docs/audit-backlog.md` (security, 2026-07-12). The CI-side half of the same question is `bun audit` in CI + the Renovate npm extension, listed with the other pending rulings below.
...
- [ ] **`update_mixtape_cue` — wire or retire.** A live admin-API op with **no caller** in the repo (the mixtape-scoped rail it was written for is unmounted; [docs/fluncle-studio.md](../fluncle-studio.md) says so plainly). Its decision waits on the same placement contract as the observation dialogs — see _The overhaul → Intake_.
...
- **Anchor worklist — finish item 4 (cheapest high-impact item left).** The `has_embedding` mirror column and its covering index already landed for the funnel scan, but `ANCHOR_ORDER` in `track-work.ts` still sorts on `(t.embedding_blob is not null) desc`, and the item's partial index `(has_embedding, nearest_finding_score) where spotify_uri is null` does not exist. This is one index and one `ORDER BY` against a mirror that is already live.
- **Rank sweep at 150k — item 3 (highest impact remaining).** Maintained `match_key` + `needs_rank` columns, so duplicate resolution seeks instead of building a full-catalogue map (~31s), and an idle tick costs O(0) instead of full-scanning the PK behind a `catalogue_rank_corpus <> ?` predicate that can never ride a btree. Hosted proof gated, per the no-local-performance-claims rule.
...
- **Before opening catalogue capture, decide capturable-flag vs isolate-merge.** Wave-2 item 6 and Wave-3 item 4 are the same decision wearing two hats (the flag is the denormalize path, the isolate-merge is the design path); the backlog says to decide them together, and neither bites until catalogue capture opens.
- **Four design calls await a decision** — the per-user recommendations cache, search `compileFilters` resolving names to indexed ids, tracks-hub keyset pagination for the deep tail, and the capture split-OR merge (`docs/db-scale-backlog.md` Wave 3). They are decisions, not builds; the ledger holds the shapes and impacts. The year-lane rollup on `/tracks` (item 5, MEDIUM) rides along with whichever of these is picked up.
