...
Two things to know before reading. **The overhaul is the headline** — a feature freeze is in effect, so `Now` and `Next` are deliberately not new-feature lists; they are the freeze-safe work (reliability, hygiene, activations) plus the overhaul's prerequisites. And **the long tails live in their own ledgers**: the nightly auditor's findings ledger ([docs/audit-backlog.md](../audit-backlog.md), forward-facing — open rows only), the DB query-shape scale backlog ([docs/db-scale-backlog.md](../db-scale-backlog.md)), and the [homogenisation evidence ledger](./homogenisation-evidence.md). A ledger row is promoted into this doc only when it is worth scheduling, and a promoted item cites the row it came from — the ledgers keep the analysis, this doc keeps the schedule.
...
- **Then go-to-market:** promoting Fluncle in the DnB communities (Reddit and friends), reaching out to record labels (the groundwork is _Next → Label outreach_), and showing up where the scene is — the **September festival** is the first dated beat, with a batch of printed stickers ready to spread (the collateral has print lead time and does not wait on the overhaul — see _Next → Printed collateral_). The mobile app is live on the App Store (approved 2026-07-29), so its announcement rides the go-to-market beats.
...
- **The Friday newsletter's own ticks.** One real 15:00 Amsterdam run confirms it fires, authors only on a non-empty window, and re-offers an unsent draft rather than double-authoring (empty `tidbits` is the designed outcome, not a fault — the authoring agent has no web access, tidbits are recall-only under a never-fabricate rail, ruled accepted 2026-08-04); the archive's galaxy grouping settles as the cluster map gains operator-named galaxies.
...
The index-economics remainder shipped 2026-08-04: two redundant indexes dropped with hosted proof (`artist_socials_artist_id_idx`, `tracks_anchor_fill_queue_idx`), the capture-priority index ruled CLOSED-KEEP (with hosted counter-evidence filed as a possible free drop, re-proof conditions in the ledger), and the `/artists` TTFB hypothesis closed against a fresh measurement (all three hubs identical). Full analysis stays in [docs/db-scale-backlog.md](../db-scale-backlog.md). What remains:
...
The free wins are in place: the long-lived container runs with `no-new-privileges`, drops every Linux capability, and adds back only the six required by its inherited s6 state bootstrap, privilege drop, and supervision path; its tokenless pre-smoke boots the real gateway on seeded scratch state without opening Discord. Every host-timer sweep already enters as `hermes`. What remains is an attended `USER hermes` cutover for the gateway container: stop it, `chown -R 10000:10000` the `/opt/data` host directory, start the candidate against the real mount, and verify state writes before unattended self-deploy resumes. Moving the token out of the gateway-readable environment is a separate, larger change, blocked on whether upstream can deliver connector credentials without process-wide environment exposure and coupled to `rebuild-hermes.sh`'s current rollback rail, which captures and replays the running container environment. The scoped token remains the server-side publish boundary, so this is defense in depth rather than a public-readiness blocker; the operating contract stays in [docs/agents/hermes-agent.md](../agents/hermes-agent.md).

### Capture fallback — the SoundCloud rung is live; watch its yield

The decision closed 2026-08-04: the weather-clean residual read ~300 unmatched/day at current throughput (telemetry-verified; bot challenges land as retryable `failed`, never `unmatched`, so the verdicts are trustworthy), which made the spike's ~12%-of-misses SoundCloud recovery worth a second source. The rung shipped (PR #1118: `scsearch` in both ladders, per-source download URL, SoundCloud failure classifier, the duration gate auto-rejecting 30s Go+ previews) and its provenance evidence banks instead of being discarded. The 07-18 bot-walled cohort turned out to no longer exist — every pre-fix unmatched row was consumed by the same-day handling plus later re-attempts. What remains is a watch, not a build: read the SoundCloud rung's per-tick tally (the ladder counters ride the capture summary) after a week or two of live running, and note that wrong `duration_ms` on vinyl-era rows stays the one residual no fallback source can fix — if it dominates, the repair is metadata re-verification.

### Printed collateral for the September festival — the dated go-to-market leg

Stickers (and QR cards if they earn a spot) to hand out at the September festival — the one go-to-market beat with an external date. It does not wait on the overhaul: design, print, and delivery carry lead time, so it is scheduled off the festival date. The honest asset inventory: the canon palette is real but hex-only (no CMYK/spot guidance exists, and Eclipse Gold + Nebula Violet both fight process printing), the traveler mark is a painted raster whose true figure is 398×488 px — print-grade to roughly 40 mm, so larger art composes around a deliberately small figure (the app-icon precedent) — there is NO existing QR pattern (one gets established here), and no wordmark treatment exists by doctrine (the cover is it). The build path is `packages/media`'s spec + render-script precedent: candidates render to a gitignored `out/`, the operator picks and orders, the picked set is committed and recorded in `docs/socials/README.md`. **Five operator inputs before design starts:** the print vendor's template requirements (bleed, vector-vs-300dpi, color profile), the die-cut shape, physical sizes + quantities, the festival date itself, and the QR ruling (target URL + whether scan attribution matters).
...
- **The search-filter pilot is SETTLED — prod runs `openai/gpt-5.6-luna` at reasoning effort `low` via OpenRouter.** The Workers-AI open models lost on the rails (deadline misses, one followed prompt-injection); Luna at effort `low` ties Haiku on every rail at ~5× less per call, and the effort ladder INVERTS on a parse task (every level above `low` is worse — deliberation "corrects" what the rails say to copy verbatim), so the pin is load-bearing: the model + effort pair live as two committed wrangler vars that travel together, and rollback is deleting both. The bench (`apps/web/scripts/bench-search-filter.ts`) remains the rerunnable harness — any model × effort in minutes, Workers-AI REST or OpenRouter, no binding. The Cloudflare AI Gateway route still charges Luna's old prices, so OpenRouter direct stays the cheap path until it reprices.
- **The context-distil is SETTLED too — prod runs `openai/gpt-5.6-luna` at effort `medium`.** Benched 2026-08-04 on a frozen 25-track corpus (`apps/web/scripts/fixtures/distil-corpus.json` + the expected-facts answer key; harness `bench-context-distil.ts`, k=3 at prod temperature): luna-medium halved the hallucination-tripwire runs against the haiku default — the surprise being that haiku _knows_ DnB and imports ungrounded memory, so Luna's ignorance is a grounding asset — used zero worn-texture vocabulary, and costs ~8× less per note; haiku's recall edge (0.88 vs 0.82) was the accepted trade for internal fuel feeding voice surfaces. The pair travels together as two wrangler vars (`OPENROUTER_CONTEXT_MODEL` + `OPENROUTER_CONTEXT_EFFORT`); rollback is deleting both. One optional top-up: the corpus is Firecrawl-only (the Apple MusicKit trio is not in the local env), so the Apple-echo grader idles until an operator-side re-capture seeds Apple-fuel cases. **Submission triage is NOT this pattern:** it runs `claude -p` with tools loading the `copywriting-fluncle` skill on subscription auth, so a Luna flip there is an invocation-path rebuild, not a bench-then-flip. The cheap lever on that surface is the already-wired `TRIAGE_CLAUDE_EFFORT` box env.
...
- **The un-parking path, whenever clips are wanted:** cue a source recording or caption a clip (the four existing clips come from the un-cued rolling set, so they build empty and are skipped), `fluncle admin clips drip-resume`, one real `drip_clips` tick to prove the push end to end — then add the `clip-drip-timer/` host unit beside `studio-clip-timer/` and let pin-watch deploy it ([docs/fluncle-studio.md](../fluncle-studio.md) § Box deploy describes the baked-scripts + host-timer mechanism).

### Fluncle mobile — 1.0 live on the App Store, the 1.1 arc unblocked

The Expo app ships feed, archive, finding pages, submissions, the Radio (background audio + lock-screen presence), Mixtapes, search, device-local saves, push toggles, and Apple Music links. **1.0 is live** — approved 2026-07-29 ([apps.apple.com/app/id6790080540](https://apps.apple.com/app/id6790080540)), the `app.ios` registry surface flipped the same day, `ascAppId` recorded in `eas.json`. The 5.2.3 remediation (`97897ed7`, the muted-feed invariant) stays load-bearing: any change that lets a feed video sound its own track is submission-blocking. Runbook: [docs/mobile-release.md](../mobile-release.md); posture: [docs/app-store-review.md](../app-store-review.md).

- [ ] **1.1 ships — accounts in the pocket.** The build landed in one orchestrated run (#595–#602, plus the hydration gap closed same-day via `list_set_tracks`); what remains is the operator's dev-client rebuild (`expo-secure-store` is native), the on-device pass (sign up → save → set → notation → delete → reset), and the submission. The submission gates are named in [docs/mobile-release.md](../mobile-release.md) § _The 1.1 checklist_: the marketing `version` is already bumped to `1.1.0` (`apps/mobile/app.config.js` + the Info.plist), App Privacy re-declared with Email Address added, the review-notes addendum, and screenshots only if a surface changed materially.
...
- **`speakable` markup.** The SpeakableSpecification pass, once assistants actually consume it — and note the intent is already achieved by other means (the definitional prose is mirrored verbatim into every JSON-LD `description`), so this stays parked until a consumer exists.
...
The machine- and developer-facing surfaces mapped in [docs/surfaces-doctrine.md](../surfaces-doctrine.md) (dig, the versioned contract-first API, the Fumadocs `/docs` hub with its per-page sitemap children, feeds, CLI distribution, SSH deep-links) are live. What's open is **the non-gating tail:** the `today` dig label, a public changelog, a Docker image, broader data-graph anchors (Discogs, Last.fm, ListenBrainz), directory listings (Product Hunt, Internet Archive, a Hugging Face dataset), the **RollDaBeats question** (the ~100k-track DnB discography corpus, dead and unarchived for 3+ years, ownerless — surfaced by the 2026-07-29 identity-graph research as an adjacent opportunity; wants its own scoping if ever picked up), the deferred **rave SSH `.onion`** identity (the web onion is live — `docs/tor.md`), and a **Discord representation of the log spine** (the one surface from the identity map never stood up; Discord exists today only as the Hermes chat presence). Each becomes a registry entry when one earns its keep.
...
- **Public marginalia RFC:** public crew cards, public submission credit, crew notes, reports, moderation, and profile-like surfaces need their own RFC before implementation. Hard default remains no public writing. (The shape sketched during the account redesign: public profiles at `/crew/<username>` — the username is the slug, the crew number rides the profile.)
...
- **The gigs / festival crawler is the load-bearing prerequisite (operator-named 2026-07-18).** Watchers are half-built without events: the fan wants "my artist just dropped" AND "my artist is playing near me." So the first real dependency is an **events subsystem** — concerts, raves, and festivals as first-class entities (an `events` table: artist(s) via the existing graph, venue, city, date, ticket link), crawled the way the catalogue is (a seeded, deterministic, resumable sweep — the [catalogue-crawler](../catalogue-crawler.md) `crawl_frontier` shape is the precedent). No such entity exists today (`schema.ts` has `status_events`, `cost_events`, `rate_limit_events` and nothing else). It stands on its own too — a public `/events` or per-artist "upcoming" surface, `MusicEvent` JSON-LD for SEO — and it is the biggest genuinely-unbuilt product slice on this list. Scope it FIRST — and the scoping is largely done: the source-availability + ToS matrix, the table/sweep shape, the SEO ruling, and the five open operator decisions are worked out in [docs/planning/events-page-spike.md](./events-page-spike.md).
...
### The DnB identity graph — the tail after the build (2026-07-29)

The identity surface shipped: the honest-state payload (the identity serializer module in `apps/web/src/lib/server`, beside `identity-dials.ts`), `get_track`'s `?identity=` / `?isrc=` / `?mbid=` keys, the `/out/spotify/<trackId>` hop, the two dials with a tested abuse alert, the public `/identity` reader page, the `/terms` fair-use + commercial-API + attribution edits, and `/docs/identity`. Posture A holds: free, no key, no money machinery, and the platform-link map stays free for as long as Fluncle holds a Spotify developer app (Developer Terms V.5, contract rather than choice). Evidence, including the Unit-0 prod measurements: [docs/planning/identity-graph-research.md](./identity-graph-research.md).

Four things remain, each on a trigger rather than a date:

- **The nightly identity dump.** No consumer, so no dump. The identity serializer it would reuse already ships, which puts it at about a day's work. Trigger: a consumer appears, label outreach the likely one.
- **The hop on the existing DTOs.** Ruling 7 wants every served-for-following Spotify link to be the hop; today only the identity payload serves it and the track DTOs still hand out raw links. The blocker is the phone: a redirect loses the OS-level app-open, so the flip waits on an iOS build that carries the one-fetch resolver, and on that build being the one people are running. JSON-LD `sameAs` keeps the raw link permanently either way, and a test pins that no hop URL can ever reach a `sameAs` array.
- **The key layer** (an `api_keys` table, `bucket = keyId`, one more auth tier). Off the critical path by design. Triggers: the abuse alert firing, a real integrator asking for headroom, or wanting the MCP / `/chat` unblock. Strictly before any money.
- **Posture B.** A paid rung where money buys bulk and convenience while the links themselves never degrade. Nothing that shipped forecloses it and nothing argues for it yet; the monetizable half was always Fluncle's OWN analysis (BPM, key, embeddings, sonic neighbours, certification), never the link map.

Two platforms answer `unsupported` to machine callers, each for a stated reason rather than a hedge: **Apple Music** on a clause read (ADPLA §3.3.6(D) confines MusicKit results to facilitating a user's own Apple Music subscription — first-party rendering on Fluncle's own pages is untouched), and **Tidal** because there is no integration at all. Re-ruling Apple is one constant in the identity serializer module. Deezer joined the covered set on 2026-07-30 (the ids the three existing Deezer reads already carried, kept and gated), and the `backfill_deezer` sweep leg now accretes the link forward over certified rows and the Ear-ranked catalogue alike (`FLUNCLE_BACKFILL_DEEZER_LIMIT`, default 25/tick — keyless, so nothing to provision, but Deezer's quota is per-IP and the Worker shares Cloudflare's egress, which is what keeps the batch small).

**Beatport** became the seventh covered platform on 2026-07-30, as the one BUY link among the five. Tiers 1 and 2 shipped together — a backfill over the certified findings, and new findings picked up by the same `fluncle-backfill` sweep leg as they publish (`backfill_beatport`, `FLUNCLE_BACKFILL_BEATPORT_LIMIT`, default 10/tick). Resolution is keyless and exact: one scrape of Beatport's public search through Firecrawl (the site is Cloudflare-walled, so a bare fetch 403s), whose results embed their own ISRCs, and the link is kept only on byte-equality with the ISRC Fluncle already holds. Three open threads, each on a trigger:

- **Tier 3, the uncertified catalogue tail, is metered rather than open.** The leg now carries a catalogue worklist behind its certified one, but the economics still rule the throttle: every row is a slow rendered scrape against a five-figure catalogue, so the tier drains a small capped batch per tick (`FLUNCLE_BACKFILL_BEATPORT_CATALOGUE_LIMIT`, a Worker var, default 5) and only on the pass that has already exhausted the certified feed. Unlike the certified tier it asks each row ONCE — a concluded row never re-spends a credit. Raising that cap is the operator's lever whenever a scrape budget makes the tail cheap.
- **The API application.** Beatport's catalogue API is partner-gated and the operator's access application is pending. An approved key REPLACES the scrape fetcher wholesale and makes tier 3 a different (much cheaper) question. The ecosystem's borrowed-embed-`client_id` workaround stays permanently forbidden — it is unauthorised access to a gated API, and no key shortage changes that.
- **No re-check cadence is ruled.** A clean miss reads `Not found · checked <date>` and promises nothing, which is why the identity payload serves `retry: "single-shot"` with `terminal: null`. Ruling a cadence is a one-constant change in `beatportState`.

The stored fields are **terminal link artifacts** under Beatport's §F (no text/data mining, no AI training on site content): a URL and a timestamp, read only by the identity payload and the `/identity` page, and barred from the FTS5 index, the LLM search tier, and every embedding. The resolver reads Beatport's key/BPM/genre off the same page object and keeps none of it.

### Database latency — the deferred levers (Placement Hints, then the own-box)

Phase 0 shipped: the query-wave collapse and the `/log` edge cache extended to the `/artist|/album|/label/<slug>` detail pages with purge-on-write, so a cache hit crosses no ocean and the reader/SEO half of the Dublin-anchor pain is off the hot path. The full six-option analysis lives in [docs/planning/turso-latency-research.md](./turso-latency-research.md). (D1 was considered and **rejected** there — it forks the store and downgrades similarity to ANN, abandoning the exact `vector_distance_cos` scan the archive is built on. Placement Hints was on this list and is **removed** — operator ruling 2026-08-03; the audit that retired it is in git history.) One lever remains, **earned by numbers, not pushed**:

- **The own-box — now specifically the WHOLE-APP port.** The vector half already took the box route (`apps/sonar`), so "put the corpus in RAM on a box" is settled and is not a fork to re-litigate. What remains is hosting the whole app on a box behind Cloudflare so the database is `localhost` — SSR + oRPC + DB in one process, with the Phase 0 cache as the global-reach engine and Litestream→R2 / Turso as the backup. A strategic architecture bet, not a config flip. The target architecture and the throwaway de-risking spike are sketched in [docs/planning/own-box-spike.md](./own-box-spike.md). Trigger: only if post-Phase-0 numbers prove the uncacheable per-user path is still too slow, or to de-risk the port early while traffic is low.
...
## Epic — search goes internal: the read path is DONE; the submit fallback is the open decision (2026-07-18, operator-ratified)

The migration the epic originally described turns out to be complete: every browse surface — web ⌘K, CLI `fluncle search`, the mobile archive tab, MCP/ChatDnB `search_archive`, the recommendations panel, the mix builder — already searches Fluncle internally. Spotify's `search_tracks` survives ONLY as the submit funnel (the web dialog, CLI `submit`, the SSH terminal, mobile submit, MCP/WebMCP feeding `submit_track`), and the crawler deliberately does NOT write `spotify_url` — anchoring is the separate breaker-gated sweep, so un-anchored catalogue rows are a standing population, and an un-anchored row cannot be submitted internally at all (`validateSubmissionInput` requires the Spotify id + matching URL). That coverage gap is what makes the remainder a product call rather than a plumbing job. What remains:

- **The operator decision that gates the rest — the submit fallback posture:** internal-first with Spotify-on-miss (smallest delta, every consumer keeps working, but Spotify stays in the anonymous path), or internal-only plus paste-a-URL (truly retires the read token, but relaxes the id-must-match-URL invariant and defers the metadata fetch to review time). The groundwork is in place: internal search resolves Spotify URLs and `spotify:` URIs locally, and a regression test locks the all-internal read path.
- **Blocked on that decision:** the name collapse (`search_archive` → the canonical `search_tracks` at `/search`) — the path and the name are occupied by the Spotify op whose fate the posture decides, and two shipped binaries (the mobile app, the SSH Go server) hard-code the current shapes, so the collapse also carries an alias/redirect call.
