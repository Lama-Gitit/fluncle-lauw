...
- **No re-check cadence is ruled.** A clean miss reads `Not found · checked <date>` and promises nothing, which is why the envelope serves `retry: "single-shot"` with `terminal: null`. Ruling a cadence is a one-constant change in `beatportState`.

The stored fields are **terminal link artifacts** under Beatport's §F (no text/data mining, no AI training on site content): a URL and a timestamp, read only by the identity envelope and the `/identity` page, and barred from the FTS5 index, the LLM search tier, and every embedding. The resolver reads Beatport's key/BPM/genre off the same page object and keeps none of it.
...
- **Unblocked now:** a Spotify-URL / `spotify:` resolution tier inside internal search keyed on `tracks.spotify_uri` (the prerequisite for any fallback posture — today a pasted URL falls to FTS and misses, while the Spotify op short-circuits it), a regression test locking the all-internal read path, and the filter-vocabulary collapse (`TracksHubFilters` already mirrors `SearchFilters` verbatim for the shared six fields; the work is a type intersection plus deleting the ~12-line `sharedFilters()` bridge).
- **The operator decision that gates the rest — the submit fallback posture:** internal-first with Spotify-on-miss (smallest delta, every consumer keeps working, but Spotify stays in the anonymous path), or internal-only plus paste-a-URL (truly retires the read token, but relaxes the id-must-match-URL invariant and defers the metadata fetch to review time).
