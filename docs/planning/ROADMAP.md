...
## Offline-first mobile — local SQLite with a Turso sync path (operator direction, 2026-07-28)

Tackled **in parallel with the overhaul** — it is mobile architecture, not a new web surface, so the freeze does not gate it. The app becomes local-first: a device-local SQLite database is the read path — snappy, and fully usable offline — syncing to Turso, which is exactly the embedded-replica / offline-sync shape Turso is built for. Scoping questions before any build: which slice syncs first (the user's own data — saves, sets, preferences, under the 1.1 union-merge law — versus a device replica of the public catalogue slice for true offline browsing); the Expo runtime path (libSQL's React Native bindings / op-sqlite); sync cadence and conflict posture (the public slice is server-authoritative, user data merges); and how the oRPC read path degrades to local when the network is gone. Composes with the 1.1 accounts-sync arc rather than replacing it — accounts decide WHAT syncs, this decides WHERE reads live. The scoping research ran 2026-07-29 — the four questions are answered with evidence in [docs/planning/offline-first-mobile-research.md](./offline-first-mobile-research.md) (headline: `expo-sqlite`'s native libSQL path, a three-slice shape, and the named first de-risk spike: `syncLibSQL()` on a real device) — so the RFC session starts from that file, not from scratch.
...
**The RFC exists: [docs/dnb-identity-graph-rfc.md](../dnb-identity-graph-rfc.md)** (Final, 2026-07-29; evidence in [docs/planning/identity-graph-research.md](./identity-graph-research.md)). The research demoted the business case (Linktree killed the canonical resolver rather than charge it; indie willingness-to-pay measures ≈ 0; Spotify's "standalone service" clause is structural, not price-gated) and promoted the asset case: the honest-negative contract no incumbent can retrofit, free by recommendation. What waits on the operator: the six-item decision sheet in the RFC — headline calls are the artifact (extend existing ops vs identity dump vs `/resolve` reader page vs a new keyed endpoint, ruled after the Unit-0 measurement runs), the posture (A recommended), the freeze timing, and a retirement trigger. Unit 0 (measurement) + Unit 1a (ledger trues-ups) are freeze-safe and runnable before any ruling.
