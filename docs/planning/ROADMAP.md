...
- [ ] **CSP graduation.** `security-headers.ts` ships an enforcing `frame-ancestors 'self'` plus a full `Content-Security-Policy-Report-Only` policy, reporting to Sentry's Security feed ([docs/error-tracking.md](../error-tracking.md)). The first watch window (2026-07-27 → 07-29) has been read and its two real findings are fixed in the policy and the client entry. Three things stand between here and the flip:
  - **One operator call: Cloudflare's RUM beacon.** `static.cloudflareinsights.com/beacon.min.js` was injected at the edge on 07-27/28 (13 reports across www and galaxy) and is absent from every response probed since. Fluncle measures with Simple Analytics, so the default answer is to confirm Web Analytics / Observatory RUM is OFF in the Cloudflare dashboard and leave the beacon un-allowlisted — enforcing then also stops a silent re-flip. Allowlisting instead costs two entries (`script-src` plus `cloudflareinsights.com` in `connect-src` for the beacon's POST).
  - **A second, longer watch window.** The first was ~2.5 days, and report-only only ever sees the surfaces visitors actually exercised. The remaining feed noise is third-party browser software (a Perplexity font injected into the page), which no policy can absorb and which is not a reason to hold the flip.
  - **Then flip**, and un-archive FLUNCLE-WEB-7 once it is confirmed silent — it is archived forever, so any future genuine `eval` violation currently groups into it unseen.
