...
- **Same pattern, next: the context-distill only.** `distilContextNote` genuinely matches the shape — OpenRouter, registry prompt (`context_distil`), no deadline (latency is a reporting axis, not a rail) — and two mechanical graders already exist in the codebase (the Apple echo gate, the texture-vocab measurer). The bench needs a **frozen** Firecrawl snippet corpus (~20–30 tracks with hand-written expected-facts per case; live re-fetches drift and make model deltas unreadable), and a flip needs one code change: `observation.ts` sends no `reasoning` field, so an `OPENROUTER_CONTEXT_EFFORT` passthrough lands with the model var, the same two-vars-travel-together discipline as search. **Submission triage is NOT this pattern:** it runs `claude -p` with tools loading the `copywriting-fluncle` skill on subscription auth, so a Luna flip there is an invocation-path rebuild, not a bench-then-flip. The cheap lever on that surface is the already-wired `TRIAGE_CLAUDE_EFFORT` box env.
