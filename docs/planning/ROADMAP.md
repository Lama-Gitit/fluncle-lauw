...
- **Pilot: the search-filter LLM — RUN 2026-08-02, and the answer is "not this surface."** The bench shipped (`apps/web/scripts/bench-search-filter.ts`, 20 golden + 12 adversarial cases against the live registry prompt) and two Workers-AI models lost to the incumbent on the rails that matter: Haiku 4.5 went 96.9% parse / 85% golden-exact / **zero** deadline misses, `llama-3.3-70b-fp8-fast` went 87.5% / 70% with **9.4% of calls blowing the 3 s deadline** and one followed prompt-injection (it named a real Pendulum track; the schema gate contained it), and `gpt-oss-120b` timed out on a quarter of the set. What survives the pilot: the bench is a rerunnable one-command harness (any `WORKERS_AI_MODEL` in minutes, REST-only, no binding), and the two follow-on candidates below have **no 3 s deadline**, which was the open models' dominant failure mode here — so the pattern is untested where it is most likely to fit, not disproven. **Round 2, same day, via OpenRouter instead of Workers AI: `openai/gpt-5.6-luna` at reasoning effort `low` TIES Haiku** — 100% parse / 85% golden-exact (identical, and the same benign miss classes) / zero rail violations / zero deadline misses / p95 1844 ms — at roughly 5× cheaper on Luna's 2026-07-31 repricing ($0.20/$1.20 vs $1/$5 per MTok). The effort ladder INVERTS on this task (`medium` 12.5% deadline misses, `xhigh`/`max` 28–34%, and deadline-free quality still _drops_ to 65% — the deep efforts overthink a parse), so the swap is only viable with effort pinned `low`, which prod cannot express today: `search-llm.ts` sends no `reasoning` field, so the runtime slice is a small env-gated effort passthrough (`OPENROUTER_REASONING_EFFORT`, the bench's own mechanism) plus the model env flip. Operator decision pending; the Cloudflare AI Gateway route still charges Luna's old prices, so OpenRouter direct is the cheap path for now.
...
**Reconcile the two filter vocabularies into one.** `/tracks` is live (`apps/web/src/routes/tracks.tsx` over `apps/web/src/lib/server/tracks-hub.ts`), and its `TracksHubFilters` (defined in `tracks-hub.ts`, re-exported by `tracks-search.ts`) is the hub's vocabulary — `bpmMin`/`bpmMax`, `yearMin`/`yearMax`, `key`, `label`, `galaxy`, plus an API-only `certified` tri-state. **The second schema already exists:** the search LLM tier emits `SearchFilters` (`packages/contracts/src/orpc/search.ts`) — `album`/`artist`/`soundsLike`/`text` and no `galaxy` — and `sharedFilters()` in `tracks-hub.ts` is the hand-written bridge between them. The work is reconciling the two into one vocabulary, not preventing a second.
