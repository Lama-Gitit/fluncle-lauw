...
The predicted risk landed where this doc said it would — and the app came through: **1.0 was approved 2026-07-29**, after two rounds. Round one was the anticipated **Guideline 5.2.3** rejection (music rights), remediated in-binary — see the 5.2.3 section for the fix and the invariant that holds it. Round two was **metadata-only 5.2.1** (album covers in the store screenshots; the binary passed), resolved by a same-day justification reply — see the 5.2.1 note. That is the shape to expect from Apple on any future submission: rejection rounds on the music question and on cover-art metadata, not a refusal of the app. Music apps that play previews and link out already exist on the store, and Fluncle's native vertical-video feed is genuine native value (not a web wrapper). Google Play review is far more lenient — approval there is near-certain; the friction worth planning around is Apple's.
...
**Not hypothetical: 1.0 was rejected under 5.2.3.** Build 4 went to review on 2026-07-13 with the feed's videos carrying a baked audio excerpt of the track, and Apple read the app as providing access to a third-party audio catalog — the exact shape that draws the _"provide documentation of your rights to this content"_ request. Remediated in commit `97897ed7` (2026-07-21), resubmitted, and approved 2026-07-29 — the remediated posture below survived review and is the ratified baseline for every later submission.

**And the metadata drew its own bite — 5.2.1 on the store screenshots.** The resubmission passed on the binary but was rejected a second time (2026-07-28) under **5.2.1 (intellectual property)** for album covers appearing in the STORE SCREENSHOTS — review scrutinises the listing assets separately from the app. The operator removed the single cover-led preview shot and replied the same day with a justification (music apps across the store show covers in list screenshots); Apple accepted it within a day. Two durable lessons: keep listing screenshots to UI-led shots where covers are incidental list content, never the hero; and if the justification argument ever stops holding, the ready-made fallback is the own-IP screenshot rig in closed PR #976 (synthetic sleeve/avatar compositions, a guarded local-only seed, a capture runbook) — reopen, merge, re-shoot.
...
Apple's rules change between submissions. Re-read the current [App Store Review Guidelines](https://developer.apple.com/app-store/review/guidelines/) at submission time. The numbers cited here (4.2, 5.2.3, 4.5.4, 5.1.1) are stable as of 2026-07.
