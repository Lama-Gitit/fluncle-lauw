...
Temporary operator checklist for validation that must run on the mixing Mac. Delete each section after it passes, and delete the file when it is empty.
...
- [ ] Require Fluncle CLI ≥0.119.0 so `admin tracks list --json` includes `keySource`. Run `brew update && brew upgrade fluncle`; an older CLI makes the dry-run propose ~35 phantom "stamp" writes and trip the max-writes fuse, and that symptom means upgrade rather than apply.
- [ ] Confirm the CLI is operator-authenticated (`fluncle recent --limit 1` works).
- [ ] Run the sync without `--apply`: `uv run --with pyrekordbox python packages/skills/fluncle-rekordbox-sync/scripts/rekordbox_sync.py`. Investigate any unexpectedly large proposal set before applying it.
...
`packages/live/scripts/m2-sender/` reads the DDJ-FLX4's MIDI and emits the live-deck transition datagram to the bridge. Validate the MIDI hardware path against the connected DDJ-FLX4 before using it in a show.

- [ ] Confirm that `mido` exposes the `DDJ-FLX4` port while Rekordbox is running.
- [ ] Set `FLUNCLE_VJ_HOST` to the M5's reachable tailnet address because the machines are on different LAN subnets. Set `FLUNCLE_VJ_PORT` to the transition port, which defaults to 9000.
- [ ] First send one transition to a loopback UDP listener (`nc -ul 9000`); then test delivery to the M5 bridge.
...
Validate these standing parameters once on the assembled rig.
...
- [ ] `MATCH_THRESHOLD` begins at `0.62`; because false positives are costlier than misses, raise it if any wrong finding resolves.
...
Optional: compare any non-DJ-graded finding that exists in Rekordbox against `DjmdContent.Key.ScaleName`. Resolve disagreements through the operator update path, which stamps `key_source: operator`.
...
The mixability baseline currently contains one set with 16 transitions. Add each real Rekordbox history session to widen the validation corpus. The extraction script emits per-track `key` + `bpm` (Rekordbox's whole-song analysis), so a set is self-contained ground truth — its transitions score directly from the emitted values, no archive join needed, non-findings included.
