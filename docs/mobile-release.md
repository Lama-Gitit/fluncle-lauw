...
- **`eas.json` — SET** (`apps/mobile/eas.json`): a `development` profile (dev client, internal), a `preview` profile (internal distribution, for ad-hoc device installs), a `production` profile (store builds), and a `production` **submit** profile carrying `ascAppId` `6790080540` (set on 1.0's approval, 2026-07-29), so `eas submit` runs without the interactive app-picker.
...
- **Apple Developer account — LIVE** (operator). **App Store Connect app record — LIVE** (operator); **1.0 is approved and on the store** (2026-07-29, [apps.apple.com/app/id6790080540](https://apps.apple.com/app/id6790080540)).
...
6. `[agent]` **Record the `ascAppId` for non-interactive submits** — DONE (2026-07-29): `submit.production.ios.ascAppId` is `6790080540` in `eas.json` (a public identifier, safe to commit), so `eas submit` runs without an interactive app-picker.
...
16. `[operator]` **Respond to any reviewer clarification** — most likely the 5.2 music-rights question, and 1.0 drew exactly that (rejected under **5.2.3** on the first pass; see the note below the kit), then a second, METADATA-only round (**5.2.1**: album covers in the store screenshots — the binary passed) that a same-day justification reply resolved (music apps on the store show covers in list screenshots; the own-IP screenshot rig sits ready in closed PR #976 if that argument ever stops holding). 1.0 was **approved 2026-07-29** after those two rounds. The notes in step 13 carry the remediated language. Approve/hold decisions are Apple's; the resubmit is `[operator]`.
