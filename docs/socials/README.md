...
| YouTube            | `@fluncle`              | https://www.youtube.com/@fluncle                                                           | Operator-managed Google identity | "Drum & bass bangers from another dimension." (links out to fluncle.com + galaxy.fluncle.com)                                                                                                                                             |
...
| Bluesky            | `@fluncle.com`          | https://bsky.app/profile/fluncle.com                                                       | Fluncle alias                    | "Drum & bass bangers from another dimension." (+ www.fluncle.com link). Verified custom-domain handle. Findings auto-post as link cards to the /log page (`apps/web/src/lib/server/bluesky.ts`) — gated on the `BLUESKY_*` Worker secrets |
...
| Last.fm            | `fluncle`               | https://www.last.fm/user/fluncle                                                           | Fluncle alias                    | Music-graph profile; About set to the tagline. Findings sync to Last.fm loves through `apps/web/src/lib/server/lastfm.ts` — gated on the `LASTFM_*` Worker secrets                                                                        |
| Discogs            | `fluncle`               | https://www.discogs.com/user/fluncle                                                       | Fluncle alias                    | Music-graph profile (user `31223733`); profile set to the bio. Release-ID enrichment runs through `discogs.ts`; findings List writes are not implemented                                                                                  |
...
| X                  | none                    | configured contact link                                                                    | Operator-managed X               | No dedicated Fluncle X profile; the configured contact link routes to the operator-managed account                                                                                                                                        |
...
The **Sign-in** column says which identity manages each profile — not the literal login. This repo is public, so the actual login emails/usernames live in 1Password alongside the keys, not here. Most profiles use a Fluncle alias; Spotify, Telegram, and YouTube use separate operator-managed identities. Mixcloud is the licensed home for the DJ mix on the [roadmap](../planning/ROADMAP.md) — mind the Featured-Artist limits before uploading. A bare duplicate `soundcloud.com/fluncle-646915409` also exists — ignore or remove it; use `/fluncle` as the canonical SoundCloud profile.
