...
Codex mirror of the Claude Code automation in `../.claude/`. Same two hooks (and the same four subagents, mirrored under `agents/` as TOML), adapted to Codex's model (see https://developers.openai.com/codex/hooks.md). Defined in `hooks.json`; scripts in `hooks/`.
