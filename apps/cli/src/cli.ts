...
function addAdminCommands(program: Command): void {
  ...
  backfill
    ...
    .action(async (options: CoverMastersOptions) => {
      const { backfillCoverMastersCommand } = await import("./commands/admin-covers");
  ...
  albums
    .command("describe")
    .description("Author the voiced bio for an album (fills an empty bio only)")
    .argument("[slug]")
    .option(
      "--queue",
      "Show the bio worklist (albums with findings but no bio), oldest first",
      false,
    )
    .option("--limit <limit>", "Number of albums to show with --queue", "50")
    .option("--bio <text>", "The voice-gated bio paragraph")
    .option("--bio-file <file>", "Read the bio from a file")
    .option(
      "--prompt-version <n>",
    ...
    .action(async (slug: string | undefined, options: BioDescribeOptions) => {
      if (options.queue) {
        const { albumsBioQueueCommand } = await import("./commands/admin-albums");
        await runEntityBioQueue("album", options, albumsBioQueueCommand);
        return;
      }

      const { describeAlbumCommand } = await import("./commands/admin-albums");
  ...
  albums
    ...
    .allowExcessArguments()
    .action(async (slug: string, options: { json: boolean }) => {
      const { draftAlbumBioCommand } = await import("./commands/admin-albums");
...
async function runBackfillCoverMasters(
  options: CoverMastersOptions,
  backfillCoverMastersCommand: typeof import("./commands/admin-covers").backfillCoverMastersCommand,
): Promise<void> {
  const limit = parseListLimit(options.limit);
  ...
  while (resolved.length + none.length + failed.length < limit) {
    ...
    if (!options.json) {
      const verb = result.dryRun ? "would resolve" : "resolved";
      console.log(
        `  …${verb} ${result.resolvedCount}; ${result.noneCount} without a source; ${result.failedCount} failed`,
      );
    }
