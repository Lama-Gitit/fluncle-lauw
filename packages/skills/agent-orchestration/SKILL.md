...
This orchestrator-and-reviewer role sits with the session's **Fable 5** driver seat; every sub-agent it delegates to runs on **Opus 5** (`claude-opus-5`, the current Opus tier). Opus 5 benchmarks within a hair of Fable at roughly half the price, so offloading execution to it concedes nothing — Fable's per-token premium is spent where it pays (holding the overview, the adversarial review, the merge judgment), never on execution volume. Sonnet has proven unreliable, so there is no tier below Opus to argue about: hold the bar with Opus 5 for the work (see `AGENTS.md`, "Picking the right models and effort for workflows and subagents").
...
**4. Delegate to worktree sub-agents.** Use `isolation: "worktree"` so each agent works on an isolated copy and parallel agents can't collide — essential when they mutate files. Worktree agents branch from the **last pushed commit**, so push first if a local-only change must be visible to them, or inline that change into the brief. **Every worktree brief's first step is `bun install --frozen-lockfile`** — a fresh worktree has no `node_modules`, and module resolution then silently falls through to the MAIN checkout's copy, so the agent tests someone else's code and reports green (bit three of five agents in one session, 2026-07-27; one placebo-green shipped a prod 500). The agent builds, runs the relevant checks (typecheck / build / test / lint), and opens a PR. Run every agent on **Opus 5** (`model: opus` — a floating alias that resolves to the current Opus tier, deliberately not pinned) per `AGENTS.md` — no cheaper tier, even for mechanical slices (a cheap agent that under-delivers costs more than it saves).
