...
| Consumer                     | File                                                                                                              | Reads                                                                                                                                                                                            |
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/status` health board       | [`apps/web/src/routes/status.tsx`](../../../apps/web/src/routes/status.tsx)                                       | `cronSurfaces()` for the cron order; each row's title + subtitle read from the surface's `title`/`statusDescription`                                                                             |
...
**The consumers split two ways, and it decides how much work a new surface is** (the authority is §1 of [docs/surfaces-doctrine.md](../../../docs/surfaces-doctrine.md)). **Automatic** consumers read the catalog at runtime, so a new entry appears with no second edit: the `/status` board, the box-side freshness prober, the MCP `get_status` + CLI `fluncle status` labels, and the post-deploy probe. **Hand-honoured** consumers only have the registry's stated intent — a human still wires them: the homepage nav + dev-row (`apps/web/src/lib/nav-model.ts`, its own model), the rave terminal menu (`menuItems()` in `apps/ssh/main.go`, a hand-written Go slice), the CLI help surface (`apps/cli/src/cli.ts`), `llms.txt` (`lib/server/agent-discovery.ts`), the sitemap (`lib/server/sitemap-data.ts`), and this doc's §2/§3 tables. `surfacesForContext` / `surfacesByWeight` / `surfacesByKind` exist for exactly those and have **no consumer outside the registry package yet** — so a `web`/`ssh`/`cli` weight is a declaration of prominence, not wiring.
...
2. **Wire the fan-out checklist** — so no consumer is forgotten:
   - [ ] **`/status` probe** — add a `probeConfig` (`http` with `cadenceMs`/`timeoutMs`, or `cron` with `cronName`/`cadenceMs`). HTTP surfaces are GET-probed; crons are checked by last-run freshness. A **cron** carries its `/status` row label in the registry itself — add `title` + `statusDescription` (REQUIRED; the registry test build-fails a cron missing either), and it renders with no `status.tsx` edit. A NON-registry core service (`web`/`db`/`r2`, a short infra alias, not a registry name) still needs its label in `status.tsx` (`INFRA_SERVICE_LABELS`/`INFRA_SERVICE_SUBTITLES`) + `SERVICE_ORDER`, and `Probed on /status as service `<id>`` in `operatorNotes`. A new cron also needs a row in `fluncle-healthcheck.ts`'s on-box mirror.
   - [ ] **nav / dev-row** — set `weights.web` for homepage prominence and add the actual entry to `apps/web/src/lib/nav-model.ts`; the nav model is hand-wired.
   - [ ] **SSH menu** — same split: `weights.ssh` states its terminal prominence, and `menuItems()` in `apps/ssh/main.go` (a hand-written Go slice) carries the actual menu row (and a deep-link if it gets one).
   - [ ] **CLI help** — same split: `weights.cli` states its help prominence if it is a `fluncle` verb, and `apps/cli/src/cli.ts` carries the actual command.
...
4. **Gated behind an external approval?** (a store review, a DNS cutover, anything not yet live). Add the entry now with **`pending: true`** — `liveSurfaces()` drops it, so it is reviewed but DARK everywhere (no menu, no `/status` probe, the dev-row, `llms.txt`, the sitemap, the §2/§3 tables). Fill in its real `weights`/`probeConfig` and a placeholder `url`, and capture the flip plan in `operatorNotes`. The day it goes live, the fan-out is **one field-flip**: drop `pending`, swap the placeholder address for the real one, and add its §2/§3 rows. **Three surfaces carry `pending: true` today** — verify the live set with `rg -n 'pending: true' packages/registry/src/index.ts`, and read each one's `operatorNotes` for its flip plan, since the gate differs:

- **`web.mix`** — gated by a self-lifting depth measurement, not approval; dark only so we do not advertise a URL that still redirects.
- **`web.chat`** and **`web.recommendations`** — live and serving, but gated to the verified-crew rollout cohort and `noindex`; their flip also drops the route's `noindex` meta.
...
The invariants test guards the catalog: unique `name`s, kind-shaped fields (a `cron` probeConfig only on a `cron` surface), and the per-context weight partition (within each context, the four weights cover exactly the surfaces displayed there, sorted loudest-first). A failing test names the violated invariant.
