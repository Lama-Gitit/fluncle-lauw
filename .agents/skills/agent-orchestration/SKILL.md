...
2. **Launch with model AND effort pinned.** `codex exec -C <worktree> --sandbox workspace-write -m gpt-5.6-sol -c model_reasoning_effort="xhigh" -o <report-file> "<brief>"` (a long brief pipes via stdin). Pinning both is load-bearing: the operator's personal `~/.codex/config.toml` defaults to a DIFFERENT model, so an unpinned call silently runs the wrong workhorse. `xhigh` is Sol's default (`max` is marginally better at twice the price — the ROI is not there); step down for mechanical slices. Do not pass `--ephemeral` — session files are what make resume work.
...
- **Verify the worktree installed before trusting its green.** An agent's "all tests pass" from a worktree without its own `node_modules` proved nothing (see step 4). Cheap check: the report mentions running `bun install`, or the diff's behavior was verified against schema-true fixtures.
- **A slice adding SQL against prod tables ships a generated-migrations integration test** that executes the actual query (the `createIntegrationDb` pattern) — pure-helper unit coverage does not discharge this. The 2026-07-26 reach-board 500 shipped through 3,952 green tests because none ran the SQL against the real schema.
- **Watch checks with `scripts/watch-checks.sh <pr#|sha>`**, not a hand-rolled loop — the ad-hoc versions produced two watcher defects in one day (a padded-SHA poll that read empty forever; a moving-HEAD watcher that expired mid-build). The script exits 0 green / 1 red / 2 undetermined, and its output is an INPUT to your merge decision, never a step before a pre-decided merge.
