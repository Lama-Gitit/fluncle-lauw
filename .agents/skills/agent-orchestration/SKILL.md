...
2. **Launch with model AND effort pinned.** `codex exec -C <worktree> --sandbox workspace-write -m gpt-5.6-sol -c model_reasoning_effort="high" -o <report-file> "<brief>"` (a long brief pipes via stdin). Pinning both is load-bearing: the operator's personal `~/.codex/config.toml` defaults to a DIFFERENT model and effort, so an unpinned call silently runs the wrong workhorse. `high` is Sol's default; step UP to `xhigh` for the most complex slices and the adversarial review pass (`max` is marginally better than `xhigh` at twice the price — the ROI is not there), and down for mechanical ones. Do not pass `--ephemeral` — session files are what make resume work.
