...
2. **Wire the fan-out checklist** — so no consumer is forgotten:
   ...
   - [ ] **nav / dev-row** — TWO edits, because `weights.web` is HAND-HONOURED: give it `weights.web` to state its homepage prominence, then carry the actual entry in the nav model (`apps/web/src/lib/nav-model.ts`), which has its own model and does not read the registry. `surfacesForContext("web")` has no consumer outside the registry package yet, so the weight alone surfaces nothing.
